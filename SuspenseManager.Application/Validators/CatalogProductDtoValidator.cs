...
public class CreateCatalogProductDtoValidator : AbstractValidator<CreateCatalogProductDto>
{
...
public class UpdateCatalogProductDtoValidator : AbstractValidator<UpdateCatalogProductDto>
{
    public UpdateCatalogProductDtoValidator()
    {
        RuleFor(x => x.ProductName)
            .NotEmpty().WithMessage("Название продукта не может быть пустой строкой")
            .MaximumLength(255).WithMessage("Название продукта не должно превышать 255 символов")
            .When(x => x.ProductName != null);

        RuleFor(x => x.Artist)
            .NotEmpty().WithMessage("Артист не может быть пустой строкой")
            .MaximumLength(255).WithMessage("Артист не должен превышать 255 символов")
            .When(x => x.Artist != null);

        RuleFor(x => x.Isrc)
            .NotEmpty().WithMessage("ISRC не может быть пустой строкой")
            .MaximumLength(15).WithMessage("ISRC не должен превышать 15 символов")
            .Matches(@"^[A-Za-z]{2}-?[A-Za-z0-9]{3}-?\d{2}-?\d{5}$")
            .WithMessage("Некорректный формат ISRC. Ожидается: CCXXXYYNNNN или CC-XXX-YY-NNNNN (например: RU-S1Z-24-00001)")
            .When(x => x.Isrc != null);
        ...
        RuleFor(x => x.Barcode)
            .NotEmpty().WithMessage("Баркод не может быть пустой строкой")
            .MaximumLength(20).WithMessage("Баркод не должен превышать 20 символов")
            .Matches(@"^\d{8}$|^\d{12}$|^\d{13}$")
            .WithMessage("Баркод должен содержать 8 цифр (EAN-8), 12 цифр (UPC-A) или 13 цифр (EAN-13)")
            .When(x => x.Barcode != null);
        ...
        RuleFor(x => x.CatalogNumber)
            .NotEmpty().WithMessage("Каталожный номер не может быть пустой строкой")
            .MaximumLength(100).WithMessage("Каталожный номер не должен превышать 100 символов")
            .When(x => x.CatalogNumber != null);

        RuleFor(x => x.ProductFormatCode)
            .NotEmpty().WithMessage("Код формата не может быть пустой строкой")
            .MaximumLength(50).WithMessage("Код формата не должен превышать 50 символов")
            .When(x => x.ProductFormatCode != null);
